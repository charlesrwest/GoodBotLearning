...
FullyConnectedLayerDefinition::FullyConnectedLayerDefinition(const FullyConnectedLayerDefinitionParameters& inputParameters) : inputBlobName(inputParameters.inputBlobName), layerName(inputParameters.layerName), weightFillType(inputParameters.weightFillType), biasFillType(inputParameters.biasFillType), activationType(inputParameters.activationType), numberOfInputs(inputParameters.numberOfInputs), numberOfNodes(inputParameters.numberOfNodes), batchSize(inputParameters.batchSize)
{
}
...
int64_t FullyConnectedLayerDefinition::GetBatchSize() const
{
return batchSize;
}
